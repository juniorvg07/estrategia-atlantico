...
export const LiderPanel = ({ lider, data1, data2, onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      setMensaje("⚠️ Por favor, completar todos los campos.");
      return;
    }
    ...
    //Envía los datos en una método put
    axiosInstance
  ...
  const handleEliminar = () => {
    if (documentoLider === "") {
      setMensaje("⚠️ No hay datos para eliminar.");
      return;
    }

    // Confirmación del usuario
    const confirmacion = window.confirm(
      `¿Estás seguro que deseas eliminar al Líder "${nombreLider}" "${apellidoLider}"?`
    );

    if (!confirmacion) {
      setMensaje("ℹ️ Eliminación cancelada.");
      return;
    }

    // Si el usuario confirma, elimina
    axiosInstance
      .delete("/personal", idLider)
      .then((res) => {
      ...
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <section className="form-button">
            <button id="delete" type="button" onClick={handleEliminar}>
              Eliminar Datos
            </button>
